    ...
    <div class="small-header margin-l-r-0 row wrapper border-bottom white-bg page-heading ng-scope">
        ...
        <div class="col-xs-8">
            ...
            <div class="title-action">
                <a href="#lead/create" class="btn btn-success btn-sm">Create a Lead</a>
...
<script>
    ...
    $('.data-table').DataTable({
        ...
        var input = $('.dataTables_filter input').unbind(),
            global_search = $("#leads-search input"), 
            self = this.api(),
        ...
        global_search.keypress(function(e) {
                if (e.which == 13) {
                    self.search(global_search.val()).draw();
                }
           }
        );
    }
...
<?php if ($_SESSION['api']['user']['permissionLevel'] == 'ADMINISTRATOR' || $_SESSION['api']['user']['permissionLevel'] == 'MANAGER'): ?>
        dom: 'lBfrtip',
                ...
                text: 'Assign',
                        ...
                        if ($('.assignLead:checked').length == 0) {

                        toastr.error('Please select a lead to assign.');
                        } else {
                        $('#assignAgentModal').modal('show');
                        }

                        },
                        className: 'btn btn-success submissionErrors'
                }
                <?php
...
<?php endif; ?>
    ...
    $('.assignLead').prop('checked', true);
    } else {

    $('.assignLead').prop('checked', false);
    }
    });
